...
export function ExperienceSection() {
	...
	return (
		<section id="experience" className="py-24 md:px-4 relative overflow-hidden">
			{/* Animated Background */}
				...
				<div className="relative">
					...
					<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
						{experience.map((exp, index) => {
							...
							return (
								<div key={index} className={`relative flex  ${isVisible ? "animate-slide-in-bounce" : "opacity-0"}`}
									style={{ animationDelay: `${index * 0.3}s` }}
								>

									{/* Experience Card */}
									<Card className={`group w-full glass-morphism border-0 hover:shadow-2xl transition-all duration-700 `} >
										{/* Animated Border */}
										<div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${gradientColor} animate-shimmer`} />

										{/* Background Gradient */}
										<div className={`absolute inset-0 bg-gradient-to-br ${gradientColor} opacity-0 group-hover:opacity-5 transition-opacity duration-500 rounded-lg`} />
